...
Home Manager links only the writable Backpass user-scope source below. The
remaining checked-in skill
...
- Matt Pocock, Vision, agent-stuff, Impeccable, No Mistakes, AXI, lavish-axi,
  gnhf, and Firstmate/stow sources are seeded from their upstream repositories
  by `home/bin/update-skills`. The `kunchenguid/firstmate` source supplies the
  stow skill; `jacobaraujo7/remote_pi` supplies selected optional skills, while
  the checked-in agent-network material remains a local compatibility adapter.
...
- `agent-network/` and `stow/` remain compatibility material for the
